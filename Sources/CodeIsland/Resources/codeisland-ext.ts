...
// ── Debug logging ─────────────────────────────────────────────────────────────
/** Set CODEISLAND_DEBUG=1 to emit stderr traces. */
const DEBUG = process.env.CODEISLAND_DEBUG === "1" || process.env.CODEISLAND_DEBUG === "true";
function dbg(msg: string, ...args: unknown[]): void {
  ...
  process.stderr.write(`[codeisland-ext ${ts}] ${msg}${extra}\n`);
}
...
function sendToSocket(payload: object): Promise<boolean> {
  const ev = (payload as Record<string, unknown>).hook_event_name ?? "?";
  dbg(`sendToSocket → ${ev} (path=${SOCKET_PATH})`);
  const inner = new Promise<boolean>((resolve) => {
    let settled = false;
    try {
      ...
      sock.on("error", (err) => {
        dbg(`  error → ${ev}: ${err.message}`);
        if (!settled) { settled = true; resolve(false); }
      });
      sock.setTimeout(2_000, () => {
        dbg(`  socket timeout → ${ev} (2s)`);
        sock.destroy();
        if (!settled) { settled = true; resolve(false); }
      });
    } catch (err) {
      dbg(`  throw → ${ev}: ${(err as Error).message}`);
      resolve(false);
    }
  });
  // Hard outer guard — never let sendToSocket exceed 5s regardless of
  // connect/accept quirks on macOS Unix sockets.
  return Promise.race([
    inner,
    new Promise<boolean>((resolve) =>
      setTimeout(() => { dbg(`  HARD TIMEOUT → ${ev} (5s)`); resolve(false); }, 5_000),
    ),
  ]);
}
...
function sendAndWaitResponse(
  ...
  const ev = (payload as Record<string, unknown>).hook_event_name ?? "?";
  dbg(`sendAndWaitResponse → ${ev} (timeout=${timeoutMs}ms, bridge=${BRIDGE_PATH})`);
  return new Promise((resolve) => {
    if (!existsSync(BRIDGE_PATH)) {
      dbg(`  bridge not found → ${ev}`);
      resolve(null);
      return;
    }
    try {
      const child = execFile(
        ...
        (error, stdout) => {
          if (error) {
            dbg(`  bridge error → ${ev}: ${(error as Error).message.slice(0, 120)}`);
            resolve(null);
            return;
          }
          try {
            ...
            dbg(`  bridge parse fail → ${ev} (stdout len=${stdout.length})`);
            resolve(null);
          }
        },
      );
      ...
      dbg(`  bridge throw → ${ev}: ${(err as Error).message}`);
      resolve(null);
    }
  });
}
...
export default function codeislandExtension(pi: ExtensionAPI) {
  ...
  async function ensureSessionStarted(sessionId: string, cwd: string): Promise<void> {
    ...
    dbg(`ensureSessionStarted → ${sid} (cwd=${cwd})`);
    const sessionName = pi.getSessionName();
    const ok = await sendToSocket(
      base(sessionId, cwd, {
        hook_event_name: "SessionStart",
        ...(sessionName ? { session_title: sessionName } : {}),
      }, tty),
    );
    dbg(`  SessionStart sent → ${sid} (ok=${ok})`);
  ...
  pi.on("tool_call", async (event, ctx) => {
    const sessionId = ctx.sessionManager.getSessionId();
    const sid = `pi-${sessionId}`;
    dbg(`tool_call → ${event.toolName} (sid=${sid})`);
    await ensureSessionStarted(sessionId, ctx.cwd);
    ...
    if (
      ...
      dbg(`  DANGEROUS bash → ${sid}, blocking via bridge: ${event.input.command.slice(0, 80)}`);
      pendingPermissionSessions.add(sid);
    ...
    dbg(`  PreToolUse → ${toolName} (sid=${sid}, pending=${pendingPermissionSessions.has(sid)})`);
